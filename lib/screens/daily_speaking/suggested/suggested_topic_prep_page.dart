...
/// at the top; the rest is a guide the learner skims to feel ready for a free
/// talk: a talk **structure** (outline), **words**, **phrases**, **grammar
/// patterns**, **common mistakes**, things to mention, and a **gated example
/// answer**. Reference sections collapse so the page stays glanceable — only the
/// outline is open by default, and the example stays closed so it doesn't spoil
/// the challenge. CTA hands the topic to the recorder.
...
/// A titled section that expands/collapses on tap. Reference material starts
/// collapsed so the prep page reads as a short, skimmable kit.
...
/// An example sentence-starter under an outline step — a quoted, accent-tinted
/// opener the learner can borrow to get the first sentence out.
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // The pattern itself, set off like a little formula.
          Container(
...
/// signal they open a detail sheet.
...
class _WordChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        label,
        style: PmpTextStyles.body2Regular.copyWith(color: colorScheme.onSurface),
      ),
    );
  }
}
