...
/// persist a new history row (the practice attempts are the v1…vN sessions;
/// this is just the model answer for the best of them). It calls the service
/// directly. Token cost for the rewrite is therefore paid exactly once, here.
...
class _FinalRewritePageState extends State<FinalRewritePage> {
  ...
  Future<DailySpeakingFeedback> _loadRewrite() {
    const sections = [
    ...
    return DailySpeakingService().reviewSession(input);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Native version')),
      body: SafeArea(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Text(
                  finalScore == null
                      ...
                      : 'You finished at $finalScore. Here\'s a native version '
                          'to compare against.',
                  style: PmpTextStyles.body1Semi
                      .copyWith(color: colorScheme.onSurface),
                ),
          ...
          // For the write path we can show the learner's own words side by side;
          // voice has no transcript so we just show the native version.
          if (learnerText != null && learnerText!.trim().isNotEmpty) ...[
            const SizedBox(height: 20),
            const _Label(text: 'Your version'),
            const SizedBox(height: 8),
            _PlainCard(text: learnerText!.trim()),
          ],
          if (feedback.nativeRewrite.isNotEmpty) ...[
            const SizedBox(height: 20),
            const _Label(text: 'Native version'),
            const SizedBox(height: 8),
            Container(
          ...
          if (feedback.sentenceRewrites.isNotEmpty) ...[
            const SizedBox(height: 20),
            const _Label(text: 'Sentence by sentence'),
            const SizedBox(height: 8),
